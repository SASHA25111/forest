/* ═══════════════════════════════════════════════════════════════
   review-v4.13.js — логика «сервиса правок» (внешний файл для Tilda)
   + метаданные проектов, админ-панель (список/очистка/закрытие)
...
(function () {
  ...
  function boot() {
    ...
    /* ── Админ-панель (список проектов / очистка / закрытие) ── */
    ...
    function loadProjects(){
      ...
      db.collection("projects").get().then(function(qs){
        ...
        Array.prototype.forEach.call(adminList.querySelectorAll(".rv__adm-close"), function(b){ b.addEventListener("click", function(){ toggleClose(b.getAttribute("data-id"), b.getAttribute("data-closed")==="1"); }); });
    ...
    function adminRow(p){
      ...
      return '<div class="rv__adm-row">'
        + '<div class="rv__adm-main"><div class="rv__adm-title">'+esc(p.title||p.id)+(closed?' <span class="rv__adm-badge">закрыт</span>':'')+'</div>'
        + '<div class="rv__adm-meta">ID: '+esc(p.id)+' · правок: <b data-count="'+esc(p.id)+'">…</b> · обновлён '+upd+'</div></div>'
        + '<div class="rv__adm-actions">'
        + '<button class="rv__adm-btn rv__adm-close" data-id="'+esc(p.id)+'" data-closed="'+(closed?"1":"0")+'">'+(closed?"Открыть":"Закрыть")+'</button>'
        + '<button class="rv__adm-btn rv__adm-danger rv__adm-clear" data-id="'+esc(p.id)+'">Очистить</button>'
        + '</div></div>';
    }
    function clearProject(id){
      if(!confirm('Удалить ВСЕ комментарии проекта "'+id+'"? Это необратимо.')) return;
      var ref=db.collection("projects").doc(id).collection("comments");
      ref.get().then(function(qs){
        if(qs.empty){ loadProjects(); return; }
        var batch=db.batch(), n=0, chain=Promise.resolve();
        qs.docs.forEach(function(d){
          batch.delete(d.ref); n++;
          if(n===450){ var b=batch; chain=chain.then(function(){ return b.commit(); }); batch=db.batch(); n=0; }
        });
        if(n>0){ var b2=batch; chain=chain.then(function(){ return b2.commit(); }); }
        chain.then(function(){ loadProjects(); }).catch(function(e){ alert("Не удалось очистить: "+e.message); });
      }).catch(function(e){ alert("Ошибка: "+e.message); });
    }
    function toggleClose(id, currentlyClosed){
      var newStatus = currentlyClosed ? "open" : "closed";
      db.collection("projects").doc(id).update({ status:newStatus, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }).then(function(){
        if(id===projectId){ projectClosed = (newStatus==="closed"); renderFormState(); }
        loadProjects();
