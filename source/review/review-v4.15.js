/* ═══════════════════════════════════════════════════════════════
   review-v4.15.js — логика «сервиса правок» (внешний файл для Tilda)
  ...
  function boot() {
    ...
    function renderAuth(){
      if(role){
        ...
        var adminBtn = role==="performer" ? ' <button class="rv__link" id="adminBtn">Админ-панель</button>' : '';
        authArea.innerHTML='<span class="rv__av">'+avatarHTML(avatar,role)+'</span> <b>'+esc(author)+'</b> · '+lbl+adminBtn+' <button class="rv__link" id="logout">Выйти</button>';
    ...
    function loadProjects(){
      ...
      db.collection("projects").get().then(function(qs){
        ...
        Array.prototype.forEach.call(adminList.querySelectorAll(".rv__adm-link"), function(b){ b.addEventListener("click", function(){ copyLink(b.getAttribute("data-id"), b); }); });
    ...
    function copyLink(id, btn){
      var url = SITE_BASE + id;
      var restore = function(){ var t=btn.getAttribute("data-lbl")||"Ссылка"; setTimeout(function(){ btn.textContent=t; },1200); };
      btn.setAttribute("data-lbl", "Ссылка");
      if(navigator.clipboard && navigator.clipboard.writeText){
        navigator.clipboard.writeText(url).then(function(){ btn.textContent="Скопировано"; restore(); }).catch(function(){ window.prompt("Ссылка на проект:", url); });
      } else { window.prompt("Ссылка на проект:", url); }
    }
