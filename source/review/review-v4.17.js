/* ═══════════════════════════════════════════════════════════════
   review-v4.17.js — логика «сервиса правок» (внешний файл для Tilda)
   + метаданные проектов, админ-панель (Ссылка / очистка / удаление)
   v4.17: видеохостинг Cloudflare Stream → Kinescope (доступен в РФ
     без VPN). Плеер через Kinescope IframePlayer SDK: create(),
     события Play/Pause/TimeUpdate/DurationChange, seekTo(). Наш
     бегунок, таймкод-комментарии и рисование работают как прежде.
   v4.16: «Ссылка» открывает страницу в новой вкладке; убрана подпись
     роли в шапке; «Админ-панель» и «Выйти» — кнопки.
   v4.15: кнопка «Ссылка» в админке (SITE_BASE + PROJECT_ID).
   v4.14: «Закрыть» → «Удалить» (проект + комменты), кастомное
   ...
   Подключение (в блоке Tilda, ПОСЛЕ этих файлов):
     ...
     firebase-auth-compat.js, Kinescope iframe.player.js
   Конфиг страницы задаётся в блоке через window.REVIEW_CONFIG.
   VIDEO_UID теперь = ID видео из Kinescope (из ссылки /embed/<ID>).
...
(function () {
  ...
  function boot() {
    ...
    var SITE_BASE     = CFG.SITE_BASE     || "https://forestproduction.ru/";  // база для кнопки «Ссылка»
    ...
    var KINESCOPE_BASE = "https://kinescope.io/"; // + VIDEO_UID
    ...
    projectRef.get().then(function(doc){
      if(!doc.exists){
        projectRef.set({ title:PROJECT_TITLE, videoUid:VIDEO_UID, status:"open", createdAt: firebase.firestore.FieldValue.serverTimestamp(), updatedAt: firebase.firestore.FieldValue.serverTimestamp() }).catch(function(){});
      } else {
        var dd=doc.data()||{}; projectClosed = dd.status==="closed";
        projectRef.update({ title:PROJECT_TITLE, videoUid:VIDEO_UID, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }).catch(function(){});
      }
      renderFormState();
    }).catch(function(){});
    ...
    function renderAuth(){
      if(role){
        var adminBtn = role==="performer" ? '<button class="rv__authbtn" id="adminBtn">Админ-панель</button>' : '';
        authArea.innerHTML='<span class="rv__av">'+avatarHTML(avatar,role)+'</span> <b>'+esc(author)+'</b>'+adminBtn+'<button class="rv__authbtn" id="logout">Выйти</button>';
        document.getElementById("logout").addEventListener("click", doLogout);
        if(role==="performer") document.getElementById("adminBtn").addEventListener("click", openAdmin);
    ...
    /* ── Кастомное подтверждение «Вы уверены? Да/Нет» ── */
    var confirmOv=document.getElementById("rvConfirm"), confirmMsg=document.getElementById("rvConfirmMsg"), confirmYes=document.getElementById("rvConfirmYes"), confirmNo=document.getElementById("rvConfirmNo");
    function askConfirm(msg){
      return new Promise(function(resolve){
        if(!confirmOv){ resolve(window.confirm(msg)); return; }
        confirmMsg.textContent = msg; confirmOv.style.display="flex";
        function done(v){ confirmOv.style.display="none"; confirmYes.removeEventListener("click", onY); confirmNo.removeEventListener("click", onN); resolve(v); }
        function onY(){ done(true); } function onN(){ done(false); }
        confirmYes.addEventListener("click", onY); confirmNo.addEventListener("click", onN);
      });
    }

    /* ── Админ-панель (список проектов / ссылка / очистка / удаление) ── */
    var adminOv=document.getElementById("rvAdmin"), adminList=document.getElementById("rvAdminList"), adminCloseBtn=document.getElementById("rvAdminClose");
    if(adminCloseBtn) adminCloseBtn.addEventListener("click", function(){ adminOv.style.display="none"; });
    function openAdmin(){ if(!adminOv) return; adminOv.style.display="flex"; loadProjects(); }
    function loadProjects(){
      adminList.innerHTML='<div class="rv__adm-empty">Загрузка…</div>';
      db.collection("projects").get().then(function(qs){
        if(qs.empty){ adminList.innerHTML='<div class="rv__adm-empty">Проектов пока нет.</div>'; return; }
        var docs=qs.docs.map(function(d){ var o=d.data()||{}; o.id=d.id; return o; });
        docs.sort(function(a,b){ var am=(a.updatedAt&&a.updatedAt.toMillis)?a.updatedAt.toMillis():0, bm=(b.updatedAt&&b.updatedAt.toMillis)?b.updatedAt.toMillis():0; return bm-am; });
        adminList.innerHTML = docs.map(adminRow).join("");
        docs.forEach(function(p){
          db.collection("projects").doc(p.id).collection("comments").get().then(function(cs){
            var el=adminList.querySelector('[data-count="'+p.id+'"]'); if(el) el.textContent=cs.size;
          }).catch(function(){});
        });
        Array.prototype.forEach.call(adminList.querySelectorAll(".rv__adm-link"), function(b){ b.addEventListener("click", function(){ openLink(b.getAttribute("data-id")); }); });
        Array.prototype.forEach.call(adminList.querySelectorAll(".rv__adm-clear"), function(b){ b.addEventListener("click", function(){ clearProject(b.getAttribute("data-id")); }); });
        Array.prototype.forEach.call(adminList.querySelectorAll(".rv__adm-delete"), function(b){ b.addEventListener("click", function(){ deleteProject(b.getAttribute("data-id")); }); });
      }).catch(function(e){ adminList.innerHTML='<div class="rv__adm-empty">Ошибка: '+esc(e.message)+' (список доступен только админу)</div>'; });
    }
    function adminRow(p){
      var upd = (p.updatedAt&&p.updatedAt.toMillis) ? fmtWhen(p.updatedAt) : "—";
      return '<div class="rv__adm-row">'
        + '<div class="rv__adm-main"><div class="rv__adm-title">'+esc(p.title||p.id)+'</div>'
        + '<div class="rv__adm-meta">ID: '+esc(p.id)+' · правок: <b data-count="'+esc(p.id)+'">…</b> · обновлён '+upd+'</div></div>'
        + '<div class="rv__adm-actions">'
        + '<button class="rv__adm-btn rv__adm-link" data-id="'+esc(p.id)+'">Ссылка</button>'
        + '<button class="rv__adm-btn rv__adm-clear" data-id="'+esc(p.id)+'">Очистить</button>'
        + '<button class="rv__adm-btn rv__adm-danger rv__adm-delete" data-id="'+esc(p.id)+'">Удалить</button>'
        + '</div></div>';
    }
    function openLink(id){ window.open(SITE_BASE + id, "_blank", "noopener"); }
    // удаление всех комментов проекта пачками, затем колбэк
    function wipeComments(id, done){
      var ref=db.collection("projects").doc(id).collection("comments");
      ref.get().then(function(qs){
        if(qs.empty){ done(); return; }
        var batch=db.batch(), n=0, chain=Promise.resolve();
        qs.docs.forEach(function(d){ batch.delete(d.ref); n++; if(n===450){ var b=batch; chain=chain.then(function(){ return b.commit(); }); batch=db.batch(); n=0; } });
        if(n>0){ var b2=batch; chain=chain.then(function(){ return b2.commit(); }); }
        chain.then(done).catch(function(e){ alert("Не удалось: "+e.message); });
      }).catch(function(e){ alert("Ошибка: "+e.message); });
    }
    function clearProject(id){
      askConfirm('Очистить все комментарии проекта «'+id+'»? Это необратимо.').then(function(ok){
        if(!ok) return;
        wipeComments(id, function(){ loadProjects(); });
      });
    }
    function deleteProject(id){
      askConfirm('Удалить проект «'+id+'» полностью — вместе со всеми комментариями?').then(function(ok){
        if(!ok) return;
        wipeComments(id, function(){
          db.collection("projects").doc(id).delete().then(function(){ loadProjects(); }).catch(function(e){ alert("Не удалось удалить: "+e.message); });
        });
      });
    }
    ...
    /* ── Плеер (Kinescope) + контролы + бегунок ──
       Kinescope IframePlayer: асинхронный API. Время и длительность
       берём из событий TimeUpdate/DurationChange (getCurrentTime()
       возвращает Promise, поэтому синхронно плеер не читаем). Перемотка
       — player.seekTo(sec). play()/pause() — как у Cloudflare. */
    ...
    (function wait(){ (window.Kinescope && window.Kinescope.IframePlayer) ? initPlayer() : setTimeout(wait,120); })();
    function initPlayer(){
      window.Kinescope.IframePlayer.create("sp", {
        url: KINESCOPE_BASE + VIDEO_UID,
        ...
        player = p;
        var E = p.Events || {};
        function d(ev){ return (ev && ev.data) ? ev.data : {}; }
        p.on(E.Ready, function(ev){ setDuration(d(ev).duration); });
        p.on(E.DurationChange, function(ev){ setDuration(d(ev).duration); });
        p.on(E.Play, function(){ playing=true; playBtn.innerHTML=SVG.pause; clearView(); });
        p.on(E.Playing, function(){ playing=true; playBtn.innerHTML=SVG.pause; });
        p.on(E.Pause, function(){ playing=false; playBtn.innerHTML=SVG.play; });
        p.on(E.Ended, function(){ playing=false; playBtn.innerHTML=SVG.play; });
        p.on(E.TimeUpdate, function(ev){
          var o=d(ev);
          liveTime = o.currentTime || 0;
          if(o.duration) setDuration(o.duration);
          if(!dragging) updateProgress(liveTime);
          if(attach && !pinned && !replyTo && !drawMode) setChip(liveTime);
        });
      }).catch(function(e){ console.warn("Kinescope init:", e); });
    }
    function setDuration(dv){ if(dv&&isFinite(dv)&&dv>0&&dv!==duration){ duration=dv; durEl.textContent=fmt(dv); renderMarks(); } }
    ...
    function seekTo(sec){ if(!player) return; try{ player.seekTo(sec); }catch(e){} liveTime=sec; if(attach && !pinned && !replyTo && !drawMode) setChip(sec); updateProgress(sec); }
    ...
    function seekFromEvent(e){
      ...
      updateProgress(t); if(player){ try{ player.seekTo(t); }catch(_){} } clearView();
    }
    ...
    function updateSendState(){ var has = ta.value.trim().length>0 || (drawMode && composeStrokes.length>0); sendBtn.disabled = !role || projectClosed || !has; }
    function renderFormState(){
      if(role && !projectClosed){ ta.disabled=false; ta.placeholder = replyTo ? "Ваш ответ…" : "Оставьте комментарий…"; }
      else if(role && projectClosed){ ta.disabled=true; ta.value=""; ta.placeholder="Проект закрыт для новых правок"; if(drawMode) setDrawMode(false); clearReply(); }
