/* ФАЙЛ: geosurv-v539.js — GEOsurvivor логика (v5.39)
   Изменения v5.39 (поверх v5.38):
   • HP мега-боссов теперь фиксированные по номеру появления (не зависят от scale/модификатора):
     1-й Октогон 1500, 2-й Грозовик 2200, 3-й Близнецы 2900 (общий пул), 4-й и далее — 3000.
     Хекса (мини-босс) без изменений — по-прежнему 60×HP-scale.

   Изменения v5.38 (поверх v5.37):
   • Контактный урон обычных мобов после 15:00: ×4 (было ×3); ×2 после 10:00 без изменений.
   • Призрак: наносит контактный урон и в режиме призрака (невидимости);
     видимая фаза после выстрела продлена на 3с (1с → 4с).

   Ранее (v5.37):
   • Дроп баффа с обычных мобов: до 3:00 — 3%, с 3:00 — 2%, после 5:00 — 1.5%, после 7:00 — 1%.

   Ранее (v5.36):
   • Контактный урон обычных мобов: ×2 после 10:00, ×3 после 15:00.
   • XP-кривая: уровни до 20 без изменений; с 21 ур. −20% и сглаживание (коэф. 0.68, степень 1.3).
   Ранее (v5.35):
   • Обычные враги: после 6:00 спавн 11/сек.
   • Призрак неуязвим, пока невидим; Часовой — щит на полкорпуса + лазер/веер;
     Художник — лабиринт (показ 2с → горит 4с → пауза 5с → новый). */
(function(){
  ...
  var UPGRADES = [
    {
      ...
      desc: function(l){ return '+5% к скорости движения (ур. '+l+')'; },
      maxLvl: 20,
      apply: function(s,l){ s.player.speedMul = 1 + 0.05*l; }
    },
    {
      id: 'firerate', type: 'passive', name: 'Скорострельность',
      desc: function(l){ return '+8% к частоте выстрелов (ур. '+l+')'; },
      maxLvl: 20,
      apply: function(s,l){ s.player.fireRateMul = 1 + 0.08*l; }
    },
    {
  ...
  // v5.29: жёсткая таблица damage-multipliers (заменяет старую формулу)
  // Для боссов от 13 и далее — линейный +2 за каждого
  var MEGA_DMG_TABLE = [0.85, 1.7, 2.0, 2.5, 3.0, 3.0, 5.0, 6.0, 8.0, 10.0, 12.0, 14.0];
  function getMegaDamageMul(n){
    if (n <= 0) return 1;
    if (n <= MEGA_DMG_TABLE.length) return MEGA_DMG_TABLE[n - 1];
    return MEGA_DMG_TABLE[MEGA_DMG_TABLE.length - 1] + 2 * (n - MEGA_DMG_TABLE.length);
  }

  // v5.33: жёсткое расписание — каждые 3 минуты, начиная с 3:00
  //   1: 3:00, 2: 6:00, 3: 9:00, 4: 12:00, ...
  var MEGA_TIMES = [180, 360, 540, 720, 900, 1080, 1260, 1440, 1620, 1800, 1980, 2160];
  function megaScheduleTime(n){
    if (n < MEGA_TIMES.length) return MEGA_TIMES[n];
    // После 12-го — тоже +3 мин на каждого следующего
    return MEGA_TIMES[MEGA_TIMES.length - 1] + 180 * (n - MEGA_TIMES.length + 1);
  }

  // v5.29: жёсткое расписание хексы — 1:10, 2:00, 5:00, далее +4 мин
  var HEX_TIMES = [70, 120, 300];
  // v5.39: фиксированные HP мега-боссов по номеру появления.
  //   1-й (Октогон) 1500, 2-й (Грозовик) 2200, 3-й (Близнецы, общий пул) 2900,
  //   4-й и далее — 3000 (включая повторных Близнецов — общий пул).
  function megaBossHp(n){
    if (n === 1) return 1500;
    if (n === 2) return 2200;
    if (n === 3) return 2900;
    return 3000;
  ...
  function hexScheduleTime(n){
    if (n < HEX_TIMES.length) return HEX_TIMES[n];
    return HEX_TIMES[HEX_TIMES.length - 1] + 240 * (n - HEX_TIMES.length + 1);
  }
  ...
  var Audio8 = (function(){
    ...
    function loadPref(){
      // v5.30: всегда OFF при загрузке страницы. Включается только клику.
      // (Если оставлять состояние в localStorage — браузер блокирует autoplay
      //  при загрузке без user-gesture, и музыка не играет до повторного клика.)
      return false;
    }
    function savePref(v){
    ...
    function preload(){
      if (!ctx){
        ...
        // v5.30: восстановление контекста после прерывания (звонок, переключение вкладок).
        // Без этого после входящего звонка звук пропадает и помогает только перезагрузка.
        try {
          document.addEventListener('visibilitychange', function(){
            if (!document.hidden && ctx && ctx.state === 'suspended' && enabled){
              ctx.resume().catch(function(){});
            }
          });
          window.addEventListener('focus', function(){
            if (ctx && ctx.state === 'suspended' && enabled){
              ctx.resume().catch(function(){});
            }
          });
        } catch(e){}
  ...
  function isBossAlive(){
    for (var i=0; i<state.enemies.length; i++){
      ...
      if (t === 'hex' || t === 'mega' || t === 'storm' || t === 'twin' || t === 'phantom' || t === 'sentinel' || t === 'painter') return true;
    }
    return false;
  }
  ...
  function spawnPhantom(){
    ...
    var hpScale = getHpScale();
    var scaledHp = megaBossHp(state.megaCount); // v5.39: фикс. HP по номеру босса
    var damageMul = getMegaDamageMul(state.megaCount);
    var p = state.player;
    var ang = Math.random() * Math.PI*2;
    state.enemies.push({
      type: 'phantom',
      x: p.x + Math.cos(ang)*600, y: p.y + Math.sin(ang)*600,
      vx: 0, vy: 0, hp: scaledHp, hpMax: scaledHp,
      dmg: def.dmg * damageMul, damageMul: damageMul,
      speed: def.speed * 0.6,
      r: def.radius - 2, color: '#9da8ff', score: def.score,
      sides: 8, rot: 0, hitFlash: 0, jitterPhase: 0,
      phaState: 'invis', phaTimer: 3.5, suppressMove: false
    });
    state.shake = 30; state.megaAnnounceTimer = 2.5;
    state.megaAnnounceName = 'ПРИЗРАК';
    Audio8.sfx('bossspawn'); Audio8.music('boss');
  }
  ...
  function spawnSentinel(){
    ...
    var p = state.player;
    var ang = Math.random() * Math.PI*2;
    var sx = p.x + Math.cos(ang)*600, sy = p.y + Math.sin(ang)*600;
    state.enemies.push({
      type: 'sentinel',
      x: sx, y: sy, vx: 0, vy: 0,
      ...
      r: def.radius, color: '#ff8c1a', score: def.score,
      sides: 8, rot: 0, hitFlash: 0, jitterPhase: 0,
      facingAngle: Math.atan2(p.y - sy, p.x - sx),
      // v5.35: щит на полкорпуса + атаки как у Октогон-Лорда (лазер + веер)
      fanCD: 3.5,
      laserCD: 5,
    ...
    state.shake = 30; state.megaAnnounceTimer = 2.5;
    state.megaAnnounceName = 'ЧАСОВОЙ';
    Audio8.sfx('bossspawn'); Audio8.music('boss');
  }

  // v5.35: генерация лабиринта Художника — случайные толстые перпендикулярные линии.
  // Строится вокруг точки (cx,cy) — обычно текущая позиция игрока.
  function buildPainterMaze(cx, cy){
    var RW = 720, RH = 540;          // полу-габариты области лабиринта
    var TH = 26;                     // толщина линий
    var segs = [];
    var nH = 5 + Math.floor(Math.random() * 3);  // 5..7 горизонтальных
    var nV = 5 + Math.floor(Math.random() * 3);  // 5..7 вертикальных
    var i;
    for (i = 0; i < nH; i++){
      var y = cy - RH + ((i + 0.5) / nH) * 2 * RH + (Math.random() - 0.5) * 60;
      var xStart = cx - RW + Math.random() * 0.35 * (2 * RW);
      var xLen   = (0.45 + Math.random() * 0.5) * (2 * RW);
      segs.push({ x1: xStart, y1: y, x2: xStart + xLen, y2: y, th: TH });
    }
    for (i = 0; i < nV; i++){
      var x = cx - RW + ((i + 0.5) / nV) * 2 * RW + (Math.random() - 0.5) * 60;
      var yStart = cy - RH + Math.random() * 0.35 * (2 * RH);
      var yLen   = (0.45 + Math.random() * 0.5) * (2 * RH);
      segs.push({ x1: x, y1: yStart, x2: x, y2: yStart + yLen, th: TH });
    }
    return segs;
  }

  // Расстояние от точки (px,py) до отрезка сегмента
  function distToSeg(px, py, s){
    ...
    var cxp = s.x1 + t*vx, cyp = s.y1 + t*vy;
    var dx = px - cxp, dy = py - cyp;
    return Math.hypot(dx, dy);
  }

  function spawnPainter(){
    var def = ENEMY_TYPES.mega;
    var hpScale = getHpScale();
    var scaledHp = megaBossHp(state.megaCount); // v5.39: фикс. HP по номеру босса
    var damageMul = getMegaDamageMul(state.megaCount);
    ...
    state.enemies.push({
      type: 'painter',
      x: p.x + Math.cos(ang)*500, y: p.y + Math.sin(ang)*500,
      vx: 0, vy: 0, hp: scaledHp, hpMax: scaledHp,
      dmg: def.dmg * damageMul, damageMul: damageMul,
      speed: 0,
      r: def.radius, color: '#ff5be8', score: def.score,
      sides: 8, rot: 0, hitFlash: 0, jitterPhase: 0,
      // v5.35: лабиринт-цикл (показ 2с → горит/дамажит 4с → пауза 5с → новый)
      mazeState: 'show', mazeTimer: 2.0,
      maze: buildPainterMaze(p.x, p.y),
      painterDmg: 14 * damageMul,
      suppressMove: true
    });
    state.shake = 30; state.megaAnnounceTimer = 2.5;
    state.megaAnnounceName = 'ХУДОЖНИК';
    Audio8.sfx('bossspawn'); Audio8.music('boss');
  }
  ...
  // ════════════════════════════════════════════════════
  // БЛИЗНЕЦЫ (v5.25) — 2 Октогон-Лорда с общим HP-пулом,
  // соединены синей лентой. Twin A — веер, Twin B — лазер.
  // Атаки в темпе обычного Октогон-Лорда.
  // ════════════════════════════════════════════════════
  function spawnTwins(){
    var def = ENEMY_TYPES.mega;
    var hpScale = getHpScale();
    // Общий пул HP — 1.5x от одного босса (всё-таки две туши, но бить можно любую)
    var scaledHp = megaBossHp(state.megaCount); // v5.39: фикс. HP (общий пул на двоих)
    // v5.29: damageMul по таблице
    var damageMul = getMegaDamageMul(state.megaCount);
    ...
    // Общий объект для пары: HP, флаг «уже наградили», угол орбиты
    var pair = {
      hp: scaledHp,
      hpMax: scaledHp,
      damageMul: damageMul,
    ...
    function makeTwin(role, ang){
      return {
        type: 'twin',
        twinRole: role,   // 'fan' | 'laser'
        twinPair: pair,
        x: p.x + Math.cos(ang) * dist,
        y: p.y + Math.sin(ang) * dist,
        vx: 0, vy: 0,
        hp: scaledHp, hpMax: scaledHp,    // зеркало pair.hp, синхронизируется в AI
        dmg: def.dmg * damageMul,
        damageMul: damageMul,
        speed: def.speed,
        r: def.radius - 4, color: def.color, score: Math.round(def.score / 2),
        sides: 8, rot: Math.random() * Math.PI*2,
        hitFlash: 0,
        jitterPhase: 0,
        // Атаки
        fanCD: role === 'fan' ? 3.5 : 0,
        laserCD: role === 'laser' ? 5 : 0,
        laserState: 'none', laserTimer: 0,
        laserTargetX: 0, laserTargetY: 0,
        laserDirX: 0, laserDirY: 0,
        suppressMove: false
    ...
    var twinA = makeTwin('fan',   baseAng);
    var twinB = makeTwin('laser', baseAng + Math.PI);
    twinA.twinOther = twinB;
    twinB.twinOther = twinA;

    state.enemies.push(twinA);
    state.enemies.push(twinB);

    state.shake = 30;
    state.megaAnnounceTimer = 2.5;
    state.megaAnnounceName = 'БЛИЗНЕЦЫ';
    Audio8.sfx('bossspawn');
    Audio8.music('boss');
  }
  ...
  function getSpawnRate(){
    ...
    else if (t < 360) base = 9.0 + ((t - 300) / 60) * 2.0; // v5.35: 5:00→6:00 разгон 9.0 → 11.0
    else              base = 11.0;                          // v5.35: после 6:00 потолок 11/сек
    return base;
  }
  ...
  function update(dt){
    ...
    // v5.33: спавн мега-боссов с проверкой «предыдущий мёртв + 5 сек»
    while (state.timeAcc >= megaScheduleTime(state.megaCount)){
      // Есть ли живой мега-босс?
      var megaAlive = false;
      for (var mi=0; mi<state.enemies.length; mi++){
        var mt = state.enemies[mi].type;
        if (mt === 'mega' || mt === 'storm' || mt === 'twin' ||
            mt === 'phantom' || mt === 'sentinel' || mt === 'painter'){
          megaAlive = true;
          break;
        }
      }
      if (megaAlive) break; // не спавним пока жив
      // Пауза 5 сек после смерти предыдущего мега-босса
      if (state.timeAcc < (state.lastMegaBossDeathTime || 0) + 5) break;

      // Всё, можно спавнить
      state.megaCount++;
      state.megaFirstDone = true;
      var modCycle = state.megaCount % 6;
      if      (modCycle === 1) spawnMega();
      else if (modCycle === 2) spawnStorm();
      else if (modCycle === 3) spawnTwins();
      else if (modCycle === 4) spawnPhantom();
      else if (modCycle === 5) spawnSentinel();
      else                     spawnPainter();
    }
    ...
    for (var k=0; k<state.enemies.length; k++){
      ...
      if (e.type === 'phantom'){
        actualSpeed = PLAYER_BASE_SPEED * p.speedMul * 0.35;
      }
      if (e.type === 'sentinel'){
        if (d < 400)      actualSpeed = -PLAYER_BASE_SPEED * p.speedMul * 0.5;
        else if (d > 600) actualSpeed = PLAYER_BASE_SPEED * p.speedMul * 0.55;
        else              actualSpeed = 0;
      }
      if (e.type === 'painter'){
        actualSpeed = 0;
      }
      // v5.25: Близнецы — орбитальное движение вокруг игрока на противоположных сторонах
      if (e.type === 'twin'){
        var pair = e.twinPair;
        if (pair){
          // Общий угол медленно вращается (1 оборот ~ 25 сек)
          if (e.twinRole === 'fan'){ // только один твин двигает angle (анти-дубль)
            pair.orbitAngle = (pair.orbitAngle || 0) + 0.25 * enemyDt;
          }
          var orbitR = 450;
          var ang = pair.orbitAngle + (e.twinRole === 'laser' ? Math.PI : 0);
          var targetX = p.x + Math.cos(ang) * orbitR;
          var targetY = p.y + Math.sin(ang) * orbitR;
          var tdx = targetX - e.x, tdy = targetY - e.y;
          var tdist = Math.hypot(tdx, tdy) || 1;
          // Перенаправляем nx/ny на орбитальную цель
          nx = tdx / tdist;
          ny = tdy / tdist;
          // Скорость = catch-up до 1.5x скорости игрока
          actualSpeed = Math.min(tdist * 2.4, PLAYER_BASE_SPEED * p.speedMul * 1.5);
        }
        e.rot += enemyDt * 0.6;
      }
      ...
      // ── БЛИЗНЕЦЫ AI (v5.25) ──
      if (e.type === 'twin'){
        // Синхронизируем e.hp с pair.hp (для отрисовки hitFlash, hpBar и т.п.)
        if (e.twinPair) e.hp = e.twinPair.hp;

        if (e.twinRole === 'fan'){
          // Веер — как у обычного Октогон-Лорда (3.5 сек кулдаун)
          e.fanCD -= enemyDt;
          if (e.fanCD <= 0){
            e.fanCD = 3.5;
            var fanShotsT = 5;
            var fanSpreadT = 0.8;
            var fanBaseT = Math.atan2(p.y - e.y, p.x - e.x);
            for (var fst=0; fst<fanShotsT; fst++){
              var fAngT = fanBaseT + (fst - (fanShotsT-1)/2) * (fanSpreadT / (fanShotsT-1));
              state.bossProjectiles.push({
                x: e.x, y: e.y,
                vx: Math.cos(fAngT) * 380, vy: Math.sin(fAngT) * 380,
                r: 7, life: 4.0, dmg: 9 * (e.damageMul || 1)
              });
            }
          }
        } else if (e.twinRole === 'laser'){
          megaLaserUpdate(e, p, enemyDt);
          // v5.30: пока стреляет — стоит на месте (раньше двигался с лучом)
          e.suppressMove = (e.laserState === 'fire');
        }
      }
      ...
      if (e.type === 'phantom'){
        ...
        if (e.phaState === 'invis'){
          if (e.phaTimer <= 0){
            e.phaState = 'reveal'; e.phaTimer = 4.0; // v5.38: +3с к видимой фазе после выстрела
            var tang = Math.random() * Math.PI*2;
            var tR = 180 + Math.random() * 100;
            e.x = p.x + Math.cos(tang) * tR;
            e.y = p.y + Math.sin(tang) * tR;
            var fb = Math.atan2(p.y - e.y, p.x - e.x);
            for (var phf=0; phf<6; phf++){
              var phAng = fb + (phf - 2.5) * 0.28;
              state.bossProjectiles.push({
                x: e.x, y: e.y,
                vx: Math.cos(phAng) * 360, vy: Math.sin(phAng) * 360,
                r: 7, life: 3.5, dmg: 9 * (e.damageMul || 1)
              });
            }
            state.shake = 10;
          }
        } else if (e.phaState === 'reveal'){
          if (e.phaTimer <= 0){ e.phaState = 'invis'; e.phaTimer = 3.5; }
        }
        e.rot += enemyDt * 0.4;
      }

      // ── ЧАСОВОЙ AI (v5.35: щит на полкорпуса, атаки как у Октогон-Лорда) ──
      if (e.type === 'sentinel'){
        // Щит всегда развёрнут в сторону игрока — заставляет заходить за спину
        var tang2 = Math.atan2(p.y - e.y, p.x - e.x);
        var diff = tang2 - e.facingAngle;
        while (diff > Math.PI)  diff -= 2*Math.PI;
        while (diff < -Math.PI) diff += 2*Math.PI;
        var rotS = 0.7 * enemyDt;
        if (Math.abs(diff) <= rotS) e.facingAngle = tang2;
        else e.facingAngle += (diff > 0 ? rotS : -rotS);

        // Веер снарядов — как у Октогон-Лорда
        e.fanCD -= enemyDt;
        if (e.fanCD <= 0){
          e.fanCD = 3.5;
          var senShots = 5;
          var senSpread = 0.8;
          var senBase = Math.atan2(p.y - e.y, p.x - e.x);
          for (var ssf=0; ssf<senShots; ssf++){
            var senAng = senBase + (ssf - (senShots-1)/2) * (senSpread / (senShots-1));
            state.bossProjectiles.push({
              x: e.x, y: e.y,
              vx: Math.cos(senAng) * 380, vy: Math.sin(senAng) * 380,
              r: 7, life: 4.0, dmg: 9 * (e.damageMul || 1)
            });
          }
        }

        // Лазер — точь-в-точь как у Октогон-Лорда
        megaLaserUpdate(e, p, enemyDt);
        e.suppressMove = (e.laserState === 'fire'); // пока стреляет — стоит

        e.rot += enemyDt * 0.2;
      }

      // ── ХУДОЖНИК AI (v5.35: лабиринт-цикл) ──
      // show 2с (только показ) → burn 4с (горит и дамажит) → gap 5с (пусто) → новый лабиринт
      if (e.type === 'painter'){
        e.mazeTimer -= enemyDt;
        if (e.mazeState === 'show'){
          if (e.mazeTimer <= 0){ e.mazeState = 'burn'; e.mazeTimer = 4.0; state.shake = 8; }
        } else if (e.mazeState === 'burn'){
          // Урон, если игрок стоит на любой линии лабиринта
          if (e.maze && p.iframes <= 0 && state.activeBuffs.invuln <= 0){
            var hitMaze = false;
            var hitR = PLAYER_RADIUS + 0;
            for (var mz = 0; mz < e.maze.length; mz++){
              var sg = e.maze[mz];
              if (distToSeg(p.x, p.y, sg) < sg.th * 0.5 + hitR){ hitMaze = true; break; }
            }
            if (hitMaze){
              if (tryDodge()){ p.iframes = 0.15; }
              else {
                p.hp -= e.painterDmg * enemyDt;
                p.hurtFlash = 0.12;
                if (p.hp <= 0){ p.hp = 0; endRound(); return; }
              }
            }
          }
          if (e.mazeTimer <= 0){ e.mazeState = 'gap'; e.mazeTimer = 5.0; }
        } else if (e.mazeState === 'gap'){
          if (e.mazeTimer <= 0){
            e.maze = buildPainterMaze(p.x, p.y);   // новый лабиринт вокруг игрока
            e.mazeState = 'show'; e.mazeTimer = 2.0;
          }
        }
        e.rot += enemyDt * 0.6;
      }
      ...
      // v5.38: Призрак наносит контактный урон и в режиме призрака (невидимости)
      var skipContact = false;
      if (!skipContact && pd2 < rsum*rsum && p.iframes <= 0 && state.activeBuffs.invuln <= 0){
        if (tryDodge()){
          p.iframes = 0.4;
        } else {
        // v5.38: рядовые мобы (tri/sq/diam) — после 10:00 бьют ×2, после 15:00 ×4
        var contactDmg = e.dmg;
        var isRegular = (e.type === 'tri' || e.type === 'sq' || e.type === 'diam');
        if (isRegular){
          if      (state.timeAcc > 900) contactDmg *= 4;  // после 15:00
          else if (state.timeAcc > 600) contactDmg *= 2;  // после 10:00
        }
    ...
    for (var pi=0; pi<state.projectiles.length; pi++){
      ...
      // столкновения (v5.17: пробитие, v5.26: щит Часового)
      for (var ej=0; ej<state.enemies.length; ej++){
        ...
        if (ddx*ddx + ddy*ddy < rs*rs){
          // v5.35: Часовой — щит на полкорпуса (±90°) фронта полностью блокирует пули
          if (em.type === 'sentinel'){
            var srcAng = Math.atan2(pr.y - em.y, pr.x - em.x);
            var sDiff = srcAng - em.facingAngle;
            while (sDiff > Math.PI)  sDiff -= 2*Math.PI;
            while (sDiff < -Math.PI) sDiff += 2*Math.PI;
            if (Math.abs(sDiff) < Math.PI / 2){
              spawnHitParticles(pr.x, pr.y, '#aaa', 4);
              pr._dead = true; em.hitFlash = 0.03;
              break;
            }
          }
  ...
  function damageEnemy(e, dmg){
    ...
    if (isCrit) dmg *= 2;
    // v5.35: Призрак невидим — полностью неуязвим (урон не проходит)
    if (e.type === 'phantom' && e.phaState === 'invis'){
      state.damageNums.push({ x: e.x, y: e.y - 4, val: 'НЕУЯЗВИМ', life: 0.5, dodge: true });
      return;
    }
    // v5.25: близнецы делят HP через twinPair — урон уходит в общий пул
    if (e.twinPair){
      e.twinPair.hp -= dmg;
      e.hp = e.twinPair.hp;
      if (e.twinOther) e.twinOther.hp = e.twinPair.hp;
    } else {
      e.hp -= dmg;
    }
    e.hitFlash = 0.08;
    ...
    if (e.hp <= 0){
      ...
      if (e.type === 'hex' || e.type === 'mega' || e.type === 'storm' || e.type === 'twin' || e.type === 'phantom' || e.type === 'sentinel' || e.type === 'painter'){
        state.lastBossDeathTime = state.timeAcc;
      }
      var isBossKill = (e.type === 'mega' || e.type === 'storm' ||
                        e.type === 'phantom' || e.type === 'sentinel' || e.type === 'painter' ||
                        (e.type === 'twin' && e.twinPair && !e.twinPair.scored));
      if (e.type === 'twin' && e.twinPair && !e.twinPair.scored){
        e.twinPair.scored = true;
        // Гарантированно зануляем HP другого близнеца — он умрёт через 1 кадр
        if (e.twinOther) e.twinOther.hp = 0;
      }
      if (isBossKill){
        Audio8.sfx('bossdie');
        ...
        // v5.33: время смерти мега-босса — для правила «5 сек до следующего»
        state.lastMegaBossDeathTime = state.timeAcc;
        // ── НАГРАДА ЗА МЕГА-БОССА (Octalord/Грозовик/Близнецы) ──
        ...
        if (e.isMegaMinion){
          ...
          // v5.37: дроп баффа с обычных мобов — до 3:00 3%, с 3:00 2%, после 5:00 1.5%, после 7:00 1%
          buffP = state.timeAcc > 420 ? 0.01 : (state.timeAcc > 300 ? 0.015 : (state.timeAcc > 180 ? 0.02 : 0.03));
          heartP = 0.02 * lateCut;
        }
        if (dropR < buffP){
  ...
  function addXP(v){
    ...
    while (p.xp >= p.xpNext){
      ...
      // v5.36: до 20 ур. — прежний пейс (0.85, степень 1.4);
      //         с 21 ур. — −20% и сглаживание хвоста (0.68, степень 1.3)
      var xpCoef = (p.level <= 20) ? 0.85 : 0.68;
      var xpExp  = (p.level <= 20) ? 1.4  : 1.3;
      p.xpNext = Math.round((5 + Math.pow(p.level, xpExp) * 2) * xpCoef * lateMul);
  ...
  function updatePaintZones(){
    if (!state.paintZones) state.paintZones = [];
    var p = state.player;
    for (var pz = state.paintZones.length - 1; pz >= 0; pz--){
      var z = state.paintZones[pz];
      z.life -= dtCached;
      if (z.life <= 0){ state.paintZones.splice(pz, 1); continue; }
      var pdx = p.x - z.x, pdy = p.y - z.y;
      var pd2 = pdx*pdx + pdy*pdy;
      if (pd2 < z.r * z.r && p.iframes <= 0 && state.activeBuffs.invuln <= 0){
        if (tryDodge()){ p.iframes = 0.15; }
        else {
          p.hp -= z.damage * dtCached;
          p.hurtFlash = 0.12;
          if (p.hp <= 0){ p.hp = 0; endRound(); return; }
        }
      }
  ...
  function render(){
    ...
    drawEnemies();
    drawTwinRibbons();
    for (var ds=0; ds<state.enemies.length; ds++){
      var dse = state.enemies[ds];
      if (dse.type === 'sentinel') drawSentinelExtras(dse);
      // v5.27: рендерим лазер у твина-laser (раньше не рисовался, поэтому казалось что он не стреляет)
      if (dse.type === 'twin' && dse.twinRole === 'laser') drawMegaLaser(dse);
    }
    drawProjectiles();
    drawBossProjectiles();
    drawReticle();
  ...
  function drawEnemies(){
    for (var i=0; i<state.enemies.length; i++){
      var e = state.enemies[i];
      // v5.26: outer save/restore — для прозрачности Призрака
      ctx.save();
      if (e.type === 'phantom' && e.phaState === 'invis') ctx.globalAlpha = 0.18;
      if (e.type === 'mega'){
        drawMega(e);
        ctx.restore();
        continue;
  ...
  function drawPaintZones(){
    if (!state.paintZones) return;
    for (var i=0; i<state.paintZones.length; i++){
      var z = state.paintZones[i];
      var t = z.life / z.maxLife;
      var pulse = 0.55 + 0.45 * Math.abs(Math.sin(state.timeAcc * 5));
      ctx.save();
      ctx.strokeStyle = 'rgba(255, 91, 232, ' + (0.7 * t + 0.2) + ')';
      ctx.lineWidth = 3;
      ctx.shadowColor = '#ff5be8';
      ctx.shadowBlur = IS_MOBILE ? 0 : 14;
      ctx.beginPath();
      ctx.arc(z.x, z.y, z.r, 0, Math.PI*2);
      ctx.stroke();
      ctx.shadowBlur = 0;
      ctx.fillStyle = 'rgba(255, 91, 232, ' + (0.16 * pulse) + ')';
      ctx.fill();
      ctx.restore();
    }
  }

  // v5.35: отрисовка лабиринта Художника
  //   show — бледная разметка (телеграф), burn — раскалённые толстые линии.
  function drawPainterMaze(){
    for (var pe = 0; pe < state.enemies.length; pe++){
      var e = state.enemies[pe];
      if (e.type !== 'painter' || !e.maze) continue;
      if (e.mazeState === 'gap') continue;
      var showing = (e.mazeState === 'show');
      ctx.save();
      ctx.lineCap = 'round';
      for (var m = 0; m < e.maze.length; m++){
        var s = e.maze[m];
        if (showing){
          // Телеграф: тонкий пунктирный контур будущей стены
          var tb = 0.35 + 0.25 * Math.abs(Math.sin(state.timeAcc * 6));
          ctx.strokeStyle = 'rgba(255, 91, 232, ' + tb + ')';
          ctx.lineWidth = s.th;
          ...
          ctx.lineWidth = 3;
          ctx.strokeStyle = 'rgba(255, 91, 232, 0.9)';
          ctx.beginPath(); ctx.moveTo(s.x1, s.y1); ctx.lineTo(s.x2, s.y2); ctx.stroke();
          ctx.setLineDash([]);
        } else {
          // Burn: раскалённая толстая линия (внешнее свечение + горячий центр)
          var pulse = 0.7 + 0.3 * Math.sin(state.timeAcc * 18);
          ctx.strokeStyle = 'rgba(255, 60, 160, ' + (0.45 * pulse) + ')';
          ctx.lineWidth = s.th + 16;
          ctx.beginPath(); ctx.moveTo(s.x1, s.y1); ctx.lineTo(s.x2, s.y2); ctx.stroke();
          ctx.shadowColor = '#ff5be8';
          ctx.shadowBlur = IS_MOBILE ? 0 : 18;
          ctx.strokeStyle = '#ff2ea0';
          ctx.lineWidth = s.th;
          ctx.beginPath(); ctx.moveTo(s.x1, s.y1); ctx.lineTo(s.x2, s.y2); ctx.stroke();
          ctx.shadowBlur = 0;
          ctx.strokeStyle = 'rgba(255, 230, 250, 0.85)';
          ctx.lineWidth = s.th * 0.4;
          ctx.beginPath(); ctx.moveTo(s.x1, s.y1); ctx.lineTo(s.x2, s.y2); ctx.stroke();
        }
      }
      ctx.restore();
  ...
  function drawSentinelExtras(e){
    // v5.35: щит на полкорпуса (полукруг ±90°), развёрнут в сторону игрока
    ctx.save();
    // сплошная «пластина» щита по фронтальному полукругу
    var shieldR = e.r + 16;
    ctx.fillStyle = 'rgba(255, 140, 26, 0.20)';
    ctx.strokeStyle = 'rgba(255, 190, 90, 0.85)';
    ctx.lineWidth = 5;
    ctx.shadowColor = '#ff8c1a';
    ctx.shadowBlur = IS_MOBILE ? 0 : 14;
    ctx.beginPath();
    ctx.arc(e.x, e.y, shieldR, e.facingAngle - Math.PI/2, e.facingAngle + Math.PI/2);
    ctx.stroke();
    ctx.shadowBlur = 0;
    // лёгкая заливка сектора для читаемости «фронта»
    ctx.beginPath();
    ctx.moveTo(e.x, e.y);
    ctx.arc(e.x, e.y, e.r + 34, e.facingAngle - Math.PI/2, e.facingAngle + Math.PI/2);
    ctx.closePath();
    ctx.fillStyle = 'rgba(255, 140, 26, 0.10)';
    ctx.fill();
    ctx.restore();
    // лазер — как у Октогон-Лорда
    drawMegaLaser(e);
  }

  function drawTwinRibbons(){
    for (var i=0; i<state.enemies.length; i++){
      var e = state.enemies[i];
      if (e.type !== 'twin' || e.twinRole !== 'fan') continue; // рисуем только с одной стороны
      var other = e.twinOther;
      if (!other || other._dead) continue;
      var pair = e.twinPair;
      var col = (pair && pair.ribbonColor) || '#5fb8ff';

      ctx.save();
      ctx.strokeStyle = col;
      ctx.lineWidth = 4;
      ctx.shadowColor = col;
      ctx.shadowBlur = IS_MOBILE ? 0 : 12;
      ctx.lineCap = 'round';
      // Волновая лента: quadratic curve с амплитудой по перпендикуляру
      var dx = other.x - e.x, dy = other.y - e.y;
      var len = Math.hypot(dx, dy) || 1;
      var perpX = -dy / len, perpY = dx / len;
      var wave = Math.sin(state.timeAcc * 3) * 22;
      var midX = (e.x + other.x) / 2 + perpX * wave;
      var midY = (e.y + other.y) / 2 + perpY * wave;
      ctx.beginPath();
      ctx.moveTo(e.x, e.y);
  ...
  var levelBadgeEl = document.getElementById('fgsLevelBadge');
  ...
  function updateMegaHUD(){
    ...
    for (var i=0; i<state.enemies.length; i++){
      ...
      if (bt === 'mega' || bt === 'storm' || bt === 'twin' || bt === 'phantom' || bt === 'sentinel' || bt === 'painter'){ boss = state.enemies[i]; break; }
    }
    if (!boss){
      ...
      // v5.25: для близнецов HP берётся из общего пула twinPair
      var bossHp    = boss.twinPair ? boss.twinPair.hp    : boss.hp;
      var bossHpMax = boss.twinPair ? boss.twinPair.hpMax : boss.hpMax;
      ...
      if (hpEl){
        var hpNow = Math.max(0, Math.round(bossHp));
        var hpMax = Math.round(bossHpMax);
        hpEl.textContent = hpNow + ' / ' + hpMax;
      }
      // динамическая подпись по типу босса
      var labelEl = megaBarEl.querySelector('.fgs__megabar-label');
      if (labelEl){
        var bossName = boss.type === 'storm'    ? 'ГРОЗОВИК'
                     : boss.type === 'twin'     ? 'БЛИЗНЕЦЫ'
                     : boss.type === 'phantom'  ? 'ПРИЗРАК'
                     : boss.type === 'sentinel' ? 'ЧАСОВОЙ'
                     : boss.type === 'painter'  ? 'ХУДОЖНИК'
                     : 'ОКТОГОН-ЛОРД';
        var dmgMulTxt = '';
  ...
  function updateHUD(){
    ...
    if (levelBadgeEl) levelBadgeEl.textContent = p.level;
  }
  ...
  function renderLB(list){
    ...
    list.slice(0, 7).forEach(function(row, i){
      ...
      // v5.31: вернули score, рядом с time и lvl
      var ts = row.time_seconds != null ? row.time_seconds : 0;
      var mm = Math.floor(ts / 60);
      var ss = ts % 60;
      var timeStr = ts > 0 ? (mm + ':' + (ss < 10 ? '0' + ss : ss)) : '—';
      var lvlStr  = row.level != null ? row.level : '—';
      li.innerHTML =
        '<span class="fgs__lb-rank">' + (i+1) + '</span>' +
        '<span class="fgs__lb-name">' + escapeHtml(row.name || '—') + '</span>' +
        '<span class="fgs__lb-score">' + row.score + '</span>' +
        '<span class="fgs__lb-time">' + timeStr + '</span>' +
        '<span class="fgs__lb-lvl">' + lvlStr + '</span>' +
  ...
  function submitScore(){
    ...
    var body = JSON.stringify({
      ...
      name: name,
      score: s,
      time_seconds: Math.floor(state.timeAcc || 0),
