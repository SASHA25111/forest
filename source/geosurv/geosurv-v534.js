(function(){
  ...
  function spawnSentinel(){
    ...
    state.enemies.push({
      ...
      beamState: 'none', beamTimer: 0, beamCD: 4, beamLockedAngle: 0,
      suppressMove: false
    });
    state.shake = 30; state.megaAnnounceTimer = 2.5;
  ...
  function getSpawnRate(){
    ...
    // v5.34: после 9:00 срезка убрана — потолок 9.0 (установлен на 5:00) держится дальше
    return base;
  }
    ...
    for (var k=0; k<state.enemies.length; k++){
      ...
      if (e.type === 'sentinel'){
        ...
        if (e.beamState === 'none'){
          e.beamCD -= enemyDt;
          if (e.beamCD <= 0){
            e.beamState = 'tele'; e.beamTimer = 1.5;
            e.beamLockedAngle = e.facingAngle;
          }
        } else if (e.beamState === 'tele'){
          e.beamTimer -= enemyDt;
          var diff2 = tang2 - e.beamLockedAngle;
          while (diff2 > Math.PI)  diff2 -= 2*Math.PI;
          while (diff2 < -Math.PI) diff2 += 2*Math.PI;
          e.beamLockedAngle += diff2 * 0.6 * enemyDt;
          if (e.beamTimer <= 0){ e.beamState = 'fire'; e.beamTimer = 0.8; }
        } else if (e.beamState === 'fire'){
          e.beamTimer -= enemyDt;
          var bdx = p.x - e.x, bdy = p.y - e.y;
          var bdist = Math.hypot(bdx, bdy);
          if (bdist > 1){
            var bAng = Math.atan2(bdy, bdx);
            var aDiff = bAng - e.beamLockedAngle;
            while (aDiff > Math.PI)  aDiff -= 2*Math.PI;
            while (aDiff < -Math.PI) aDiff += 2*Math.PI;
            if (Math.abs(aDiff) < 0.18 && bdist < 1400 && p.iframes <= 0 && state.activeBuffs.invuln <= 0){
              if (tryDodge()){ p.iframes = 0.3; }
              else {
                Audio8.sfx('playerhit');
                p.hp -= 14 * (e.damageMul || 1);
                p.iframes = 0.6; p.hurtFlash = 0.25; state.shake = 10;
                if (p.hp <= 0){ p.hp = 0; endRound(); return; }
              }
            }
          }
          if (e.beamTimer <= 0){ e.beamState = 'none'; e.beamCD = 4; }
        }
        e.rot += enemyDt * 0.2;
      }
      ...
      if (e.type === 'painter'){
        ...
        if (e.paintCD <= 0){
          e.paintCD = 1.8;
          var pang3 = Math.random() * Math.PI*2;
          var pdist3 = 60 + Math.random() * 200;
          state.paintZones.push({
            x: p.x + Math.cos(pang3) * pdist3,
            y: p.y + Math.sin(pang3) * pdist3,
            r: 110, life: 8.0, maxLife: 8.0,
            damage: 14 * (e.damageMul || 1)
          });
        }
        e.rot += enemyDt * 0.6;
      }
      ...
      if (!skipContact && pd2 < rsum*rsum && p.iframes <= 0 && state.activeBuffs.invuln <= 0){
        ...
        // v5.34: после 9:00 рядовые мобы (tri/sq/diam) бьют на 50% сильнее
        var contactDmg = e.dmg;
        var isRegular = (e.type === 'tri' || e.type === 'sq' || e.type === 'diam');
        if (isRegular && state.timeAcc > 540) contactDmg *= 1.5;
        Audio8.sfx('playerhit');
        p.hp -= contactDmg;
        p.hurtFlash = 0.25;
        p.iframes = 0.6;
  ...
  function drawSentinelExtras(e){
    ...
    ctx.beginPath();
    ctx.moveTo(e.x, e.y);
    ctx.arc(e.x, e.y, e.r + 30, e.facingAngle - Math.PI/3, e.facingAngle + Math.PI/3);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
    if (e.beamState === 'tele'){
      ...
      ctx.beginPath();
      ctx.moveTo(e.x, e.y);
      ctx.lineTo(e.x + Math.cos(e.beamLockedAngle) * 1400,
                 e.y + Math.sin(e.beamLockedAngle) * 1400);
      ctx.stroke();
      ctx.setLineDash([]);
    } else if (e.beamState === 'fire'){
