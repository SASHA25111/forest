(function(){
  ...
  var UPGRADES = [
    ...
    },
    {
      ...
      desc: function(l){ return '+5 к макс. HP и хил (ур. '+l+')'; },
      maxLvl: 20,
      apply: function(s,l){
        var prev = s.player.hpMax;
        s.player.hpMax = PLAYER_BASE_HP + 5*l;
  ...
  var ENEMY_CAP = IS_MOBILE ? 150 : 400;
  ...
  function freshState(){
    return {
      ...
      player: {
        ...
        speedMul: 1, fireRateMul: 1, dmgMul: 1, magnetMul: 1,
  ...
  var HP_RATE1  = IS_MOBILE ? 0.013  : 0.01;     // фаза разгона (мобила +30% vs ПК)
  var HP_RATE2  = IS_MOBILE ? 0.0013 : 0.001;    // фаза плато
  var HP_PIVOT  = IS_MOBILE ? (1 + 120 * 0.013) : 2.2;  // значение на 120с
  var HP_CAP    = IS_MOBILE ? 4.0    : 3.5;
  function getHpScale(){
    var t = state.timeAcc;
  ...
  function spawnEnemyAt(type, x, y){
    ...
    state.enemies.push({
      ...
      hp: scaledHp,
      hpMax: scaledHp,
      dmg: def.dmg, speed: def.speed * spdScale,
  ...
  function getSpawnRate(){
    ...
    var t = state.timeAcc;
    if (t < 60)  return 0.8 + (t / 60) * 1.9;          // 0.8 → 2.7
    if (t < 90)  return 2.7 + ((t - 60) / 30) * 0.2;   // 2.7 → 2.9
    if (t < 120) return 2.9 + ((t - 90) / 30) * 0.3;   // 2.9 → 3.2
    if (t < 180) return 3.2 + ((t - 120) / 60) * 0.8;  // 3.2 → 4.0
    if (t < 240) return 4.0 + ((t - 180) / 60) * 1.0;  // 4.0 → 5.0
    if (t < 300) return 5.0 + ((t - 240) / 60) * 4.0;  // 5.0 → 9.0
    return 9.0;
  }
  ...
  function computeAim(){
    var aim = state.aim;
    // приоритет — тач (twin-stick), затем мышь
    if (aimTouch.active){
      aim.x = state.camX - W/2 + aimTouch.x;
      aim.y = state.camY - H/2 + aimTouch.y;
      aim.manual = true;
    } else if (mouseAim.active){
      aim.x = state.camX - W/2 + mouseAim.x;
      aim.y = state.camY - H/2 + mouseAim.y;
      aim.manual = true;
    } else {
      aim.manual = false;
    }
  ...
  function update(dt){
    ...
    // Босс-хексы:
    //   • Первая волна — на 1:30 (по timeAcc > 90)
    //   • Следующие волны — каждые 5 минут (300с)
    //   • Количество хексов в волне растёт по 1 + floor(timeAcc/120)
    state.hexTimer += dt;
    // Хекса по расписанию:
    //   • 1-я волна — на 1:30 (ФИКС, игнорирует canSpawnBoss)
    //   • 2-я волна — на 3:00 (ФИКС, игнорирует canSpawnBoss)
    //   • 3+ — каждые 2 минуты от предыдущей (с обычной очередью)
    var hexReady = false;
    var hexForced = false;
    if (state.hexCount === 0)       { hexReady = (state.timeAcc > 90);  hexForced = true; }
    else if (state.hexCount === 1)  { hexReady = (state.timeAcc > 180); hexForced = true; }
    else                            { hexReady = (state.hexTimer > 120); }
    if (hexReady && (hexForced || canSpawnBoss())){
      state.hexCount++;
      state.hexFirstDone = true;
      state.hexTimer = 0;
      // Всегда 1 хекс за волну.
      spawnEnemyOffscreen('hex');
    }

    // ── МЕГА-БОСС «Октогон-лорд» ──
    // Первый — на 5:00 (timeAcc > 300), далее каждые 6 мин (360с)
    // Анти-коллизия с хексой: не спавнить если хекса
    // появилась < 60с назад или появится < 60с (по 60с буфер).
    state.megaTimer += dt;
    // Мега-босс по расписанию:
    //   • 1-й — на 4:00 (фикс)
    //   • далее каждые 6 минут
    var megaReady = (state.megaCount > 0)
        ? (state.megaTimer > 240)
        : (state.timeAcc > 240);
    if (megaReady){
      var megaForced = (state.megaCount === 0);  // 1-я мега в 4:00 — ФИКС
      var HEX_BUFFER = 60;
      var tooCloseToHex = false;
      if (state.hexFirstDone){
        if (state.hexTimer < HEX_BUFFER) tooCloseToHex = true;            // хекса только спавнилась
        if ((300 - state.hexTimer) < HEX_BUFFER) tooCloseToHex = true;    // хекса сейчас будет
      } else {
        var toFirstHex = 90 - state.timeAcc;
        if (toFirstHex > 0 && toFirstHex < HEX_BUFFER) tooCloseToHex = true;
      }
      if (megaForced || (!tooCloseToHex && canSpawnBoss())){
        state.megaCount++;
        state.megaFirstDone = true;
        state.megaTimer = 0;
        // Чередование: нечётные → Octalord, чётные → Грозовик
        if (state.megaCount % 2 === 1){
          spawnMega();   // Octalord (1, 3, 5...)
        } else {
          spawnStorm();  // Грозовик (2, 4, 6...)
        }
      }
      // если слишком близко — не сбрасываем megaTimer, продолжаем
      // проверять каждый кадр; как только окно расходится — спавним
    }
    ...
    for (var k=0; k<state.enemies.length; k++){
      ...
      // Вблизи (<200px) — 82% от скорости игрока,
      // далеко (>1000px) — 200%, между — линейно.
      var actualSpeed = e.speed;
      if (e.type === 'hex'){
        var playerSpeedCur = PLAYER_BASE_SPEED * p.speedMul;
        var NEAR = 200, FAR = 1000;
        var tt = Math.max(0, Math.min(1, (d - NEAR) / (FAR - NEAR)));
        actualSpeed = playerSpeedCur * (0.82 + (2.0 - 0.82) * tt);
      }
  ...
  function damageEnemy(e, dmg){
    ...
    if (e.hp <= 0){
      ...
      if (e.type === 'mega' || e.type === 'storm'){
        ...
        // Drop-логика: 3% бафф (7% для миньонов меги) / 2% сердечко / XP
        var dropR = Math.random();
        var buffDrop = (e.isMegaMinion || e.isStormMinion) ? 0.07 : 0.03;
        if (dropR < buffDrop){
        ...
        } else if (dropR < buffDrop + 0.02){
          state.hearts.push({ x: e.x, y: e.y, heal: 15 });
        } else {
          var xpVal = e.score;
          state.gems.push({ x: e.x, y: e.y, xp: xpVal });
        }
      }
      e._dead = true;
    }
  ...
  function updateMegaHUD(){
    ...
    if (!boss){
      ...
      if (labelEl){
        ...
        labelEl.textContent = 'МЕГА-БОСС · ' + bossName;
      }
    }
  ...
  function openLevelUp(){
    ...
    function pickAndResume(u, lvl){
      ...
      state.paused = false;
      p.iframes = 1.5;   // полторы секунды неуязвимости после выбора
  ...
  // ════════════════════════════════════════════════════
  var lbPcEl = document.getElementById('fgsLbPc');
  var lbMobileEl = document.getElementById('fgsLbMobile');
  var lbRefresh = document.getElementById('fgsLbRefresh');
  var lastSubmittedScore = -1;

  function fetchLeaderboard(){
    lbPcEl.innerHTML = '<li class="fgs__lb-empty">Загружаю…</li>';
    lbMobileEl.innerHTML = '<li class="fgs__lb-empty">Загружаю…</li>';
    // Просим топ-100 без dedup — клиент сделает per-device дедуп и split
    var url = APPS_SCRIPT_URL + '?action=top&game=' + GAME_KEY + '&top=100&nodedup=1';
    fetch(url, { method: 'GET' })
      ...
      .catch(function(){
        lbPcEl.innerHTML = '<li class="fgs__lb-empty">Не удалось загрузить</li>';
        lbMobileEl.innerHTML = '<li class="fgs__lb-empty">Не удалось загрузить</li>';
      });
  }

  function renderLB(list){
    if (!list || !list.length){
      lbPcEl.innerHTML = '<li class="fgs__lb-empty">Пусто</li>';
      lbMobileEl.innerHTML = '<li class="fgs__lb-empty">Пусто</li>';
      return;
    }
    var myName = '';
    try { myName = (localStorage.getItem(STORAGE_KEY) || '').toLowerCase(); } catch(e){}

    // Per-device дедуп: один и тот же ник попадает в свой топ один раз
    var pcRows = [], mobileRows = [];
    var seenPc = {}, seenMob = {};
    for (var i=0; i<list.length; i++){
      var row = list[i];
      var device = row.device === 'mobile' ? 'mobile' : 'desktop';
      var nameKey = String(row.name || '').trim();
      if (!nameKey) continue;
      if (device === 'mobile'){
        if (seenMob[nameKey]) continue;
        seenMob[nameKey] = true;
        if (mobileRows.length < 7) mobileRows.push(row);
      } else {
        if (seenPc[nameKey]) continue;
        seenPc[nameKey] = true;
        if (pcRows.length < 7) pcRows.push(row);
      }
      if (pcRows.length >= 7 && mobileRows.length >= 7) break;
    }
    renderLBColumn(lbPcEl, pcRows, myName);
    renderLBColumn(lbMobileEl, mobileRows, myName);
  }

  function renderLBColumn(el, list, myName){
    if (!list.length){
      el.innerHTML = '<li class="fgs__lb-empty">Пусто</li>';
      return;
    }
    el.innerHTML = '';
