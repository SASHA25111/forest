/* ФАЙЛ: geosurv-v535.js — GEOsurvivor логика (v5.35)
   Изменения v5.35:
   1. Обычные враги: после 6:00 спавн 11/сек; контактный урон ×2 после 6:00, ×3 после 12:00.
   2. Дроп баффа с обычных мобов после 6:00 — 1% (было 1.5%).
   3. С 30-го уровня требуемый XP урезан на 20%.
   4. Призрак полностью неуязвим, пока невидим (раньше получал 30% урона).
   5. Часовой: щит на полкорпуса (±90°) полностью блокирует урон спереди;
      луч-лазер и веер-выстрелы — по образцу Октогон-Лорда.
   6. Художник: рисует лабиринт из толстых перпендикулярных линий —
      показ 2с → горит и дамажит 4с → пауза 5с → новый лабиринт. */
(function(){
  ...
  function update(dt){
    ...
    for (var k=0; k<state.enemies.length; k++){
      ...
      if (!skipContact && pd2 < rsum*rsum && p.iframes <= 0 && state.activeBuffs.invuln <= 0){
        ...
        // v5.35: рядовые мобы (tri/sq/diam) — после 6:00 бьют ×2, после 12:00 ×3
        var contactDmg = e.dmg;
        var isRegular = (e.type === 'tri' || e.type === 'sq' || e.type === 'diam');
        if (isRegular){
          if      (state.timeAcc > 720) contactDmg *= 3;
          else if (state.timeAcc > 360) contactDmg *= 2;
        }
    ...
    if (e.hp <= 0){
      ...
      if (isBossKill){
        ...
        if (e.isMegaMinion){
          ...
          // v5.35: дроп баффа с обычных мобов после 6:00 — 1% (было 1.5%)
          buffP = state.timeAcc > 360 ? 0.01 : 0.03;
          heartP = 0.02 * lateCut;
        }
        if (dropR < buffP){
  ...
  function addXP(v){
    ...
    while (p.xp >= p.xpNext){
      ...
      var lvl30Mul = (p.level >= 30) ? 0.8 : 1.0; // v5.35: с 30 ур. нужно XP −20%
      p.xpNext = Math.round((5 + Math.pow(p.level, 1.4) * 2) * 0.85 * lateMul * lvl30Mul);
