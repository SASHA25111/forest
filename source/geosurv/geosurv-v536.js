/* ФАЙЛ: geosurv-v536.js — GEOsurvivor логика (v5.36)
   Изменения v5.36 (поверх v5.35):
   • Дроп баффа с обычных мобов: до 2:00 — 2%, с 2:00 — 1.5%, после 7:00 — 1%.
   • Контактный урон обычных мобов: ×2 после 10:00, ×3 после 15:00 (было 6:00/12:00).
   • XP-кривая: уровни до 20 без изменений; с 21 ур. −20% и сглаживание (коэф. 0.68, степень 1.3).

   База v5.35:
   1. Обычные враги: после 6:00 спавн 11/сек.
   4. Призрак полностью неуязвим, пока невидим.
   5. Часовой: щит на полкорпуса (±90°) блокирует урон спереди; лазер+веер как у Октогон-Лорда.
   6. Художник: лабиринт из толстых линий — показ 2с → горит 4с → пауза 5с → новый. */
(function(){
  ...
  function update(dt){
    ...
    for (var k=0; k<state.enemies.length; k++){
      ...
      if (e.type === 'phantom'){
        ...
        if (e.phaState === 'invis'){
          if (e.phaTimer <= 0){
            e.phaState = 'reveal'; e.phaTimer = 1.0;
      ...
      // v5.26: Призрак невидим — без контактного урона
      var skipContact = (e.type === 'phantom' && e.phaState === 'invis');
      if (!skipContact && pd2 < rsum*rsum && p.iframes <= 0 && state.activeBuffs.invuln <= 0){
        if (tryDodge()){
          p.iframes = 0.4;
        } else {
        // v5.36: рядовые мобы (tri/sq/diam) — после 10:00 бьют ×2, после 15:00 ×3
        var contactDmg = e.dmg;
        var isRegular = (e.type === 'tri' || e.type === 'sq' || e.type === 'diam');
        if (isRegular){
          if      (state.timeAcc > 900) contactDmg *= 3;  // после 15:00
    ...
    if (e.hp <= 0){
      ...
      if (isBossKill){
        ...
        if (e.isMegaMinion){
          ...
          // v5.36: дроп баффа с обычных мобов — до 2:00 2%, с 2:00 1.5%, после 7:00 1%
          buffP = state.timeAcc > 420 ? 0.01 : (state.timeAcc > 120 ? 0.015 : 0.02);
          heartP = 0.02 * lateCut;
        }
        if (dropR < buffP){
