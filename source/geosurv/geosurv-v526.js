(function(){
  ...
  function spawnPhantom(){
    ...
    var damageMul = (state.megaCount >= 2 ? 2 * Math.pow(1.3, state.megaCount - 2) : 1) * 0.85;
    var p = state.player;
  ...
  function spawnMega(){
    ...
    // v5.26: дамаг растёт со 2-го босса, минус 15% от прежней формулы
    //   1-й = 0.85, 2-й = 1.7, 3-й = 2.21, 4-й = 2.87, 5-й = 3.73 ...
    var damageMul = (state.megaCount >= 2
      ? 2 * Math.pow(1.3, state.megaCount - 2)
      : 1) * 0.85;
    var p = state.player;
    var ang = Math.random() * Math.PI*2;
    var dist = 700;
  ...
  function render(){
    ...
    for (var ds=0; ds<state.enemies.length; ds++){
      if (state.enemies[ds].type === 'sentinel') drawSentinelExtras(state.enemies[ds]);
    }
    drawProjectiles();
    drawBossProjectiles();
    drawReticle();
