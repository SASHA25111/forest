(function(){
  ...
  function spawnMega(){
    ...
    var damageMul = state.megaCount >= 4
      ? 2 * Math.pow(1.3, state.megaCount - 4)
      : 1;
    var p = state.player;
  ...
  function update(dt){
    ...
    for (var k=0; k<state.enemies.length; k++){
      ...
      if (e.type === 'hex'){
        ...
        actualSpeed = playerSpeedCur * (0.70 + (2.0 - 0.70) * tt);
      }
  ...
  function renderPlayerStats(){
    ...
    if (!el) return;
    var p = state.player;
    function pct(mul){ return '+' + Math.round((mul - 1) * 100) + '%'; }
    function pctRaw(v){ return Math.round(v * 100) + '%'; }
    function fmt2(v){ return (v * 100).toFixed(1) + '%'; }
    var w = state.weapons;
    var stats = [
      { name: 'HP',              value: Math.max(0, Math.round(p.hp)) + ' / ' + p.hpMax, maxed: false, zero: false },
      { name: 'Скорость',        value: pct(p.speedMul),        maxed: p.speedMul >= 1.75, zero: p.speedMul === 1 },
      { name: 'Скорострельность',value: pct(p.fireRateMul),     maxed: p.fireRateMul >= 2.30, zero: p.fireRateMul === 1 },
      { name: 'Урон',            value: pct(p.dmgMul),          maxed: p.dmgMul >= 2.00, zero: p.dmgMul === 1 },
      { name: 'Магнит',          value: pct(p.magnetMul),       maxed: p.magnetMul >= 3.00, zero: p.magnetMul === 1 },
      { name: 'Криты',           value: pctRaw(p.critChance),   maxed: p.critChance >= 0.20, zero: p.critChance === 0 },
      { name: 'Уворот',          value: fmt2(p.dodgeChance),    maxed: p.dodgeChance >= 0.10, zero: p.dodgeChance === 0 },
      { name: 'Вампиризм',       value: fmt2(p.vampMul),        maxed: p.vampMul >= 0.03, zero: p.vampMul === 0 },
      { name: 'Орб. щиты',       value: w.orbit.level  + ' / 2', maxed: w.orbit.level  >= 2, zero: w.orbit.level  === 0 },
      { name: 'Молния',          value: w.chain.level  + ' / 5', maxed: w.chain.level  >= 5, zero: w.chain.level  === 0 },
      { name: 'Хлыст',           value: w.whip.level   + ' / 5', maxed: w.whip.level   >= 5, zero: w.whip.level   === 0 },
      { name: 'Пушка (снаряды)', value: (w.pistol.extra + 1)+'', maxed: w.pistol.extra >= 4, zero: w.pistol.extra === 0 }
    ];
    var html = '';
