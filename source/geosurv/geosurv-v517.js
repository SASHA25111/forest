(function(){
  ...
  function getSpawnRate(){
    ...
    // v5.17: после 9:00 — обычных мобов в 2 раза меньше
    if (t > 540) base *= 0.5;
    return base;
  }
  ...
  function update(dt){
    ...
    for (var k=0; k<state.enemies.length; k++){
      ...
      if (pd2 < rsum*rsum && p.iframes <= 0 && state.activeBuffs.invuln <= 0){
        ...
        // v5.17: после 9:00 рядовые мобы (tri/sq/diam) бьют в 2 раза сильнее
        var contactDmg = e.dmg;
        var isRegular = (e.type === 'tri' || e.type === 'sq' || e.type === 'diam');
        if (isRegular && state.timeAcc > 540) contactDmg *= 2;
        p.hp -= contactDmg;
        p.hurtFlash = 0.25;
        p.iframes = 0.6;
        state.shake = 12;
